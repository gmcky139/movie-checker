...
- 3館の取得状態を保持し、取得成功館の上映だけを公開する原子的更新
...
| `real`   | 3館の公式上映予定をビルド時に取得。取得成功館をPagesで公開           | 必要         |
...
`real`モードでは3館を並列取得します。1館または2館が失敗しても、成功館の上映を公開し、失敗館には「上映情報を現在取得できません」と表示します。失敗館をサンプルデータで補完せず、全3館が失敗した場合だけ取得処理を失敗させます。
...
`.github/workflows/deploy-pages.yml`は、`main`へのpush、手動実行、6時間ごとの定期実行で起動します。`DATA_MODE=real`で3館の取得、TMDBポスター照合、データ検証、Lint、整形確認、テスト、型チェック、本番ビルドを順に実行します。個別館の取得失敗は状態を記録して成功館のデータをデプロイします。全館取得失敗または後続処理の失敗時はデプロイせず、直前に成功したPagesを維持します。
...
作品名はUnicode NFKC、空白・括弧の統一、確実に識別できる上映形式の分離、明示エイリアスによって正規化します。TMDBとの照合は候補タイトルの完全一致、管理された別名、公開年、必要な明示overrideに限定し、検索結果の先頭、人気度、曖昧一致だけでは採用しません。照合できた作品はTMDB IDと検証済み`image.tmdb.org`のポスターURLだけを生成JSONへ保存します。通常映画の取得率が70%未満ならwarningを記録して公開を継続し、未一致作品には誤った画像ではなく作品名付きのローカルfallbackを表示します。TMDB一致が0件の場合や認証・データ検証の異常はビルドを失敗させます。
...
- 個別館を取得できない場合、その映画館は取得不能と表示され、上映データは掲載されません。HTTP 403を回避・成功扱い・リトライせず、公式情報が再取得可能になるまで部分公開します。
