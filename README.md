...
> **重要:** GitHub Pagesは`sample`モードで公開しています。`real`モードの情報も正確性を保証せず、予定は変更される場合があります。購入前に必ず各映画館の公式サイトで再確認してください。
...
- `sample`: ローカルの生成データを使用します。デフォルトであり、GitHub Pagesもこのモードです。
- `real`: 次の公式上映予定から、今日を含む4日分をビルド時に取得します。3館すべての取得・解析・検証が成功しなければビルドを失敗させ、既存JSONを空データで置換しません。
...
ワークフローは`sample`固定で`npm ci`、データ生成・検証、Lint、整形確認、テスト、ビルドを順に行い、成功した`dist`をPages artifactとしてデプロイします。定期実行では生成データをリポジトリへcommitせず、その回の公開成果物だけを更新します。

`.github/workflows/validate-real-data.yml`は手動実行専用です。実データ取得と全検証を行い、JSONと`dist`を短期間のartifactとして保存しますが、GitHub Pagesへはデプロイしません。実データ公開は、各サイトの利用条件を人間が確認し、別途明示的に許可するまで無効です。
...
- 実データ公開前に、対象3サイトの利用条件、robots.txt、許容されるアクセス頻度を人間が確認する必要があります。
...
- 実データのGitHub Pages公開と定期取得は現在無効です。
